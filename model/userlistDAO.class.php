...
class userlistDAO implements CrudDAO{
    ...
    public function save($savable){
        ...
        $sql="INSERT INTO userlist(frist_name,designation,email,password,is_deleted) 
        VALUES (?,?,?,?,?)";
    ...
    public function update($updatable){
        ...
        $sql="UPDATE userlist SET frist_name='?',designation='?',email='?',password='?',is_deleted='?' WHERE id='$id'";
        $stmt = $this->dbconnection->connect()->prepare($sql);
        $stmt->execute([$frist_name,$designation,$email,$password,$is_deleted]);
    }
